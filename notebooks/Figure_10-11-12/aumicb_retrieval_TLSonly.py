...
fit_info = retriever.get_default_fit_info(
    Rs=0.8 * R_sun, Mp=50.0 * M_earth,
    Rp=0.045 * 0.82 * R_sun, T=200.0, CO_ratio=0.59, #logMp=np.log10(6.5 * M_earth),
    logZ=-1., log_cloudtop_P=-3.999, cloud_frac=1.,
...
#fit_info.add_uniform_fit_param('Mp', 5*M_earth, 15*M_earth)
...
#fit_info.add_uniform_fit_param("logZ", 0, 3)
#fit_info.add_uniform_fit_param("log_cloudtop_P", -0.99, 5) # 10^parameter Pa
#fit_info.add_uniform_fit_param("log_cloudtop_P", 0, 7) # 10^parameter Pa
...
with open(pfolder+'aumicb_nlive1000_contam_newdataDec28_errinfl_TLSonly_newT.pkl', "wb") as f:
    pickle.dump(result, f)

plotter = Plotter()
plotter.plot_retrieval_transit_spectrum(result, prefix=pfolder+
                                        'aumicb_nlive1000_contam_newdataDec28_errinfl_TLSonly_newT')
plotter.plot_retrieval_corner(result, filename=pfolder+
                              'aumicb_nlive1000_contam_newdataDec28_errinfl_TLSonly_newT_corner.pdf')
